...
にコメントとして書き、下の一覧にも記載してください。解除に判断が要るものは `tasks/goal/` に
...
上限が実際に最新 version を抑えている場合は、解除タスクを `tasks/goal/` に持たせます。
...
- [v0.4.1 の Chrome Web Store 公開を確認する](tasks/goal/chrome-web-store-v0.4.1-rollout.md)
...
記載します。今すぐ着手しない作業候補は新しい `tasks/goal/` ファイル、仕組みとして残す価値のある
