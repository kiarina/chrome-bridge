...
import type { AriaProps, AriaRegex, AriaRole, AriaTemplateNode, AriaTemplateRoleNode, AriaTemplateTextNode } from './ariaSnapshotTypes';
...
export function renderAriaTree(ariaNode: AriaNode, options?: { mode?: 'raw' | 'regex', ids?: Map<Element, number>, refs?: Map<Element, string> }): string {
  const lines: string[] = [];
  ...
  const visit = (ariaNode: AriaNode | string, parentAriaNode: AriaNode | null, indent: string) => {
    ...
    if (ariaNode.selected === true)
      key += ` [selected]`;
    if (options?.refs) {
      const ref = options.refs.get(ariaNode.element);
      if (ref)
        key += ` [ref=${ref}]`;
    }
    if (options?.ids) {
    ...
    if (!ariaNode.children.length && ariaNode.url === undefined) {
      lines.push(escapedKey);
    } else if (ariaNode.url === undefined && ariaNode.children.length === 1 && typeof ariaNode.children[0] === 'string') {
      ...
      lines.push(escapedKey + ':');
      if (ariaNode.url !== undefined)
        lines.push(indent + '  - /url: ' + yamlEscapeValueIfNeeded(ariaNode.url));
      for (const child of ariaNode.children || [])
